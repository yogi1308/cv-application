...
function ProjectsInfo({setProject, index, projects}) {
    function addProject() {
        const newEntry = { id: Date.now(), ...emptyProject };
        setProject(prev => [
    ...
    function deleteProject() {
        setProject(prev => {
            if (prev.length === 1) {return [{ id: Date.now(), ...emptyProject }]}
    ...
    function handleFieldChange(property, e) {
        if (property === 'additionalInfo') {
            setProject(prev => 
                prev.map((project, idx) =>
                    idx === index ? { ...project, additionalInfo: `${e}` } : project
                )
            )
            return
        }
        if (property === 'hide') {
            setProject(prev => 
                prev.map((project, idx) =>
                    idx === index ? { ...project, [property]: !project[property] } : project
                )
            )
        ...
        setProject(prev => 
            prev.map((project, idx) =>
                idx === index ? { ...project, [property]: e.target.value } : project
            )
        )
    }
    return (
            ...
            <div className="project-name">
                <div className="project-name-header">
                    ...
                    <div className="add-or-delete">
                            <div className="add-icon" onClick={() => addProject()} style={{ fontSize: '2rem' }} title="Add" >+</div>
                            <div className="delete-icon"  onClick={() => deleteProject()} style={{ fontSize: '1.5rem'}} title="delete" >⨂</div>
                            <div className="hide" onClick={(e) => {handleFieldChange('hide', e)}} > <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"><path d={!projects[index]?.hide ? eyeOpenPath : eyeClosedPath}/></svg></div>
                            <div className="school-dropdown" onClick={(e) => {e.target.classList.toggle('rotated'); e.currentTarget.closest('.project-name').nextElementSibling.classList.toggle('close')}} style={{ fontSize: '1.5rem' }}>▾</div>
                    </div>
                </div>
                <input type="text" name='project' id='project' autoComplete="off" onChange={(e) => handleFieldChange('projectName', e)}/>
            </div>
            <div className="other-project-info">
                <div className="project-type">
                    <h2>Project Type</h2>
                    <input type="text" name='project-type' id='project-type' placeholder='Eg. Class Project' autoComplete="off" onChange={(e) => handleFieldChange('projectType', e)} />
                </div>
                <div className="project-role">
                    <h2>Project Role</h2>
                    <input type="text" name='project-role' id='project-role' placeholder='Eg. Documentation Lead' autoComplete="off" onChange={(e) => handleFieldChange('projectRole', e)}/>
                </div>
                <div className="project-time">
                    <h2>Time</h2>
                    <input type="text" name="project-time" id="project-time" placeholder='Eg. Spring 2024' autoComplete="off" onChange={(e) => handleFieldChange('projectTimePeriod', e)}/>
                </div>
