const emptyActivity = {
  activityName: '',
  activityLocation: '',
  activityTitle: '',
  activityTimePeriodFrom: '',
  activityTimePeriodTo: '',
  additionalInfo: '',
  hide: false
}

const eyeOpenPath = "M480-320q75 0 127.5-52.5T660-500q0-75-52.5-127.5T480-680q-75 0-127.5 52.5T300-500q0 75 52.5 127.5T480-320Zm0-72q-45 0-76.5-31.5T372-500q0-45 31.5-76.5T480-608q45 0 76.5 31.5T588-500q0 45-31.5 76.5T480-392Zm0 192q-146 0-266-81.5T40-500q54-137 174-218.5T480-800q146 0 266 81.5T920-500q-54 137-174 218.5T480-200Zm0-300Zm0 220q113 0 207.5-59.5T832-500q-50-101-144.5-160.5T480-720q-113 0-207.5 59.5T128-500q50 101 144.5 160.5T480-280Z";
const eyeClosedPath = "m644-428-58-58q9-47-27-88t-93-32l-58-58q17-8 34.5-12t37.5-4q75 0 127.5 52.5T660-500q0 20-4 37.5T644-428Zm128 126-58-56q38-29 67.5-63.5T832-500q-50-101-143.5-160.5T480-720q-29 0-57 4t-55 12l-62-62q41-17 84-25.5t90-8.5q151 0 269 83.5T920-500q-23 59-60.5 109.5T772-302Zm20 246L624-222q-35 11-70.5 16.5T480-200q-151 0-269-83.5T40-500q21-53 53-98.5t73-81.5L56-792l56-56 736 736-56 56ZM222-624q-29 26-53 57t-41 67q50 101 143.5 160.5T480-280q20 0 39-2.5t39-5.5l-36-38q-11 3-21 4.5t-21 1.5q-75 0-127.5-52.5T300-500q0-11 1.5-21t4.5-21l-84-82Zm319 93Zm-151 75Z"

let showWholeSection = true

function ActivitiesInfo({setActivities, index, activities}) {
    function addActivity() {
        const newEntry = { id: Date.now(), ...emptyActivity };
        setActivities(prev => [
            ...prev.slice(0, index + 1), newEntry, ...prev.slice(index + 1)
        ])
        console.log('click')
    }

    function deleteActivity() {
        setActivities(prev => {
            if (prev.length === 1) {return [{ id: Date.now(), ...emptyActivity }]}
            else {return [...prev.slice(0, index), ...prev.slice(index + 1)]}
        })
    }

    function handleFieldChange(property, e) {
        if (property === 'additionalInfo') {
            setActivities(prev => 
                prev.map((Activity, idx) =>
                    idx === index ? { ...Activity, additionalInfo: `${e}` } : Activity
                )
            )
            return
        }
        if (property === 'hide') {
            setActivities(prev => 
                prev.map((activity, idx) =>
                    idx === index ? { ...activity, [property]: !activity[property] } : activity
                )
            )
            showWholeSection = !showWholeSection
            return
        }
        setActivities(prev => 
            prev.map((Activity, idx) =>
                idx === index ? { ...Activity, [property]: e.target.value } : Activity
            )
        )
    }
    return (
        <div className="other-experience">
            <div className='club-name'>
                <div className="activity-name-header">
                    <h2>Program/Club Name</h2>
                    <div className="add-or-delete" style={{paddingRight: '0.25rem'}}>
                        <div className="add-icon" onClick={() => addActivity()} style={{ fontSize: '2rem' }} title="Add" >+</div>
                        <div className="delete-icon"  onClick={() => deleteActivity()} style={{ fontSize: '1.5rem'}} title="delete" >⨂</div>
                        <div className="hide" onClick={(e) => {handleFieldChange('hide', e)}} > <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px"><path onClick={(e) => {handleFieldChange('hidden', e)}} d={!activities[index]?.hide ? eyeOpenPath : eyeClosedPath}/></svg></div>
                        <div className="school-dropdown" onClick={(e) => {e.target.classList.toggle('rotated'); e.currentTarget.closest('.club-name').nextElementSibling.classList.toggle('close')}} style={{ fontSize: '1.5rem' }}>▾</div>
                    </div>
                </div>
                <input type="text" name="club" id="club" autoComplete="off" onChange={(e) => handleFieldChange('activityName', e)}/>
            </div>
            <div className="other-activity-info">
                <div className="club-location">
                    <h2>Program/Club Location</h2>
                    <input type="text" name="club-location" id="club-location" autoComplete="off" onChange={(e) => handleFieldChange('activityLocation', e)} />
                </div>
                <div className="role">
                    <h2>Role</h2>
                    <input type="text" name='role' id='role' autoComplete="off" onChange={(e) => handleFieldChange('activityTitle', e)}/>
                </div>
                <div className="Time-period">
                    <h2>Time Period</h2>
                    <div className="from-to">
                        <input type="text" name="club-time-from" id="club-time-from" placeholder='From' autoComplete="off" onChange={(e) => handleFieldChange('activityTimePeriodFrom', e)}/>
                        <input type="text" name="club-time-to" id="club-time-to" placeholder='To' autoComplete="off" onChange={(e) => handleFieldChange('activityTimePeriodTo', e)}/>
                    </div>
                </div>
                <div className="more-info">
                    <h2>
                        <p className="info-on-more-info">
                            Provide highlights about your experience, including your roles and responsibilities, key projects or initiatives you worked on, technologies utilized, and any notable achievements or contributions to the organization.
                            <br /> <br />You can Bold, Italicize, Underline or add bullet points as you see fit
                        </p>
                        <span>Description</span> <span>⨁</span>
                    </h2>
                    <div id="editor" contenteditable="true"
                    onFocus={(e) => {if (e.target.innerHTML === "●	Attended weekly meetings to gain experience in Computer Science and Software Engineering topics") {e.target.textContent = ""}; e.target.style.color = 'var(--primary-text-color)' }}
                    onBlur={(e) => {if (e.target.textContent.trim() === '') {e.target.innerHTML = "●	Attended weekly meetings to gain experience in Computer Science and Software Engineering topics"; e.target.style.color = 'grey' }}}
                    onInput={e => {let html = e.currentTarget.innerHTML; html = html.replace(/^(?:<br\s*\/?>)+/i, ''); handleFieldChange('additionalInfo', html);}}>
                        ●	Attended weekly meetings to gain experience in Computer Science and Software Engineering topics
                    </div>
                </div>
            </div>
        </div>
    )
}

export default ActivitiesInfo